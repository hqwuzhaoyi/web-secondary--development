...
//
export const newJSBridge = (context) =>{
  "use strict";
  function JSBridge() {
    this.callbackDict = {};
    this.notificationIdCount = 0;
    this.notificationDict = {};
    if (context.document.addEventListener)
      context.document.addEventListener('DOMContentLoaded', function () {
      }, false);
    else
      context.document.attachEvent('DOMContentLoaded', function () {
      }, false);
  }
  JSBridge.prototype = {
    ...
    popNotificationObject: function (notificationId) {
      ...
      try {
        window.androidJS.runOnAndroidJavaScript(result);
      } catch (e) {
      }
      return result;
    },
