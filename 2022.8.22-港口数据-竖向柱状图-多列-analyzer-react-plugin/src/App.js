...
export default class App extends Component {
   constructor(props) {
      ...
      // 柱形数据排列顺序
      this.barOrder = external["柱形数据排列顺序"] || "南京港,镇江港,苏州港";
   ...
   handleEchartsData() {
      ...
      let xAxisData = [];
      // 图例数据
      let legendData = this.barOrder.split(",");
      // 首次处理数据
      let dataList = [];
      // 二次处理数据
      let seriesData = [];
      ...
      propsData.forEach((item, index) => {
         if (index > 0) {
            ...
            if (index == 1) {
               newPropsData[0] = new Array();
               newPropsData[0].push(item);
            } else {
               if (item[0] != newPropsData[indexNum][0][0]) {
                  indexNum += 1;
                  newPropsData[indexNum] = new Array();
                  newPropsData[indexNum].push(item);
               } else {
                  newPropsData[indexNum].push(item);
               }
            }
      ...
      newPropsData.forEach((item) => {
         ...
         // 动态补植
         temporaryArr.forEach((z, index) => {
            if (!z.length) {
               temporaryArr[index] = [_item[0][0], legendData[index], ""];
            }
         });

         item[0] = temporaryArr[0];
         item[1] = temporaryArr[1];
         item[2] = temporaryArr[2];
      });
      ...
      newPropsData.forEach((item) => {
         item.forEach((e) => {
            concatPropsData.push(e);
         });
      });

      concatPropsData.unshift(propsData[0]);

      let _legendData = [];

      concatPropsData.forEach((item, index) => {
         if (index > 0) {
            if (_legendData.includes(item[1])) {
                  ...
                  list.push(
                     `<div style="font-size: 14px; color: #fff; padding-left: 15px; padding-top: 2px;">
                        <div style="display: flex; align-items: center;">
                          ${params[i].value ? `<div style="width: 18px; border: 1px solid ${params[i].color}; margin-right: 10px;"></div>` : ""}
                          ${params[i].value ? `${params[i].seriesName}: ${params[i].value}` : ""}
                        </div>
                      </div>
                    `
                  );
