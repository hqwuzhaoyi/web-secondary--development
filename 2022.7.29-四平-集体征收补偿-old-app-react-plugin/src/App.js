...
export default class App extends Component {
  ...
  componentDidMount() {
    ...
    this.queryAllById();
    let search = window.location && window.location.search ? qs.parse(window.location?.search) : {};
  ...
  queryBasicInfoById = async () => {
    ...
    try {
      let { data } = await (!this.query.data_id
        ? this.query.objectid ? applicationsService.queryBasicInfoById(
          this.query.objectid,
      ...
      this.setState({
        ...
        if (
          ...
          type !== 'copyData' &&
          data?.flowInstance?.current_flow_inst_task?.current_flow_component?.shape !== 'start'
        ) {
          // await this.queryFlowDiagram();
        }

        await this.initData(data?.flowInstance?.process_status, data?.form_id, data?.flowInstance?.content_id);
  ...
  onAddEvent = async (type, button, process_status) => {
    ...
    const collectivevariable = { ...this.Form.current.sumit() }
    ...
    // if (collectivevariable.err || collectivevariable.err2) {
    //   message.error('请效验必填');
    //   return
    // }// if (!collectivevariable.placement_method) {
    //   message.error('请效验必填');
    //   return
    // }
    // console.log(this.Form.current.state.arrObj, '===============form');

    if (!collectivevariable.house_id || !collectivevariable.split_name || !collectivevariable.split_idcard || !collectivevariable.placement_method || !collectivevariable.if_deduction) {

      message.error('请效验必填');
      this.setState({ saveloading: false });
      return
    }
    if (collectivevariable.actual_address_qycj) {
      let reg = new RegExp("[\\u4E00-\\u9FFF]+", "g");
      if (reg.test(collectivevariable.actual_address_qycj)) {
        let arr = collectivevariable.actual_address_qycj.split("/");
        // 映射省
        const Provres = await applicationsService.getProvinceArea()

        for (let i = 0; i < Provres.data.length; i++) {
          let item = Provres.data[i]
          if (item.name === arr[0]) {
            arr[0] = item.id
            const Areares = await applicationsService.getAreaByParent(item.id)

            for (let y = 0; y < Areares.data.length; y++) {
              let e = Areares.data[y]
              if (e.name === arr[1]) {
                arr[1] = e.id
                collectivevariable.actual_address_qycj = arr.join(',')
              }
            }
        ...
        //         arr[0] = item.id
        //         // 映射市
        //         applicationsService.getAreaByParent(item.id).then((res1) => {
        //           res1.data.forEach((e) => {
        //             if (e.name === arr[1]) {
        //               arr[1] = e.id
        //               collectivevariable.actual_address_qycj = arr.join(',')

        //               // temptestFn(data, saveloading, components, collectivevariable)
        //             }
        //           });
        //         });
        //       }
        ...
        collectivevariable.actual_address_qycj = collectivevariable.actual_address_qycj?.join(',')
        // temptestFn(data, saveloading, components, collectivevariable)
        //           collectivevariable.actual_address_qycj[0] + "," + collectivevariable.actual_address_qycj[1];
      }
    }
  ...
  onChangeForm = async (type, button, flow, reporting) => {
    ...
    let formData = getSaveDefaultValues(components, defaultValues);
    let flow_inst_id =
      data?.flowInstance?.current_flow_inst_task?.flow_inst_id;
    let showType = {};
  ...
  actionBtnClick = (item, process_status) => {
    if (
      ...
      if (item?.type === 'save') {
        this.setState({ btnObj: item }, () => this.onAddEvent(process_status ? 'update' : 'save', item));
      } else {
      ...
      this.setState({
        ...
        formVisible: item.flowComponentItemConfig?.surface_type === 'internal',
        btnObj: item,
  ...
  handleAndModify = async item => {
    ...
    const collectivevariable = { ...this.Form.current.sumit() }
    let formData = getSaveDefaultValues(components, collectivevariable, true);
    ...
    const handle = async () => {
      ...
      try {
        await applicationsService.handleAndModify(
          params
        );
        this.setState({
          saveloading: false,
  ...
  initData = async (flag, form_id, id) => {
    const { data } = await applicationsService.queryComponents((form_id || this.query?.formid || this.query?.form_id));
    if (flag) {
      const { data: _data } = await applicationsService.queryApplyDataDetailFormId({ formId: (form_id || this.query?.formid || this.query?.form_id), id: id || this.query.data_id })
      this.setState({ components: JSON.parse(data)?.formColumnList, defaultValues: getDefaultValues(JSON.parse(data)?.formColumnList || [], _data || {}) })
    } else {
      this.setState({ components: JSON.parse(data)?.formColumnList, defaultValues: getDefaultValues(JSON.parse(data)?.formColumnList || [], {}) })
    }
  }
  ...
  buttons = () => {
    ...
    const pendingApply = ['pendingApply', 'revoked', 'beatBack', 'start'].includes(
      ...
      handleBtns.map((item, index) => {
        res.push(
          <Button
            ...
            onClick={() => {
              if (item.type === 'apply') {
                this.onAddEvent(!process_status ? "saveAndApply" : 'updateAndApply', item, process_status);
    ...
    if (pendingApply && _show === 1) {
      res.push(
        <Button onClick={() => this.printClick(data, printConfig)}>
          打印
        </Button>
      );
    }
      ...
      <Spin spinning={false} className="my-spin">
        <div className="event-details">
          <div className="content">
            {errType === 0 && (
              <ErrMessage url="end" text={'该任务已处理'} />
            )}
            {errType === 1 && (
              <ErrMessage
                url="not-found"
            ...
            >
              {defaultValues && <Form defaultValue={defaultValues} ref={this.Form} processStatus={this.state.data?.flowInstance?.process_status} />}
            </div>
          </div>
        </div>
      </Spin>
