...
class ButtonModal extends Component {
  ...
  change = () => {
    ...
    let search =
      window.location && window.location.search
        ...
        : {};
    const { processStatus, setInfo, editInfo, viewId, form } =
      this.props;
  ...
  sureChapter = () => {
    ...
    accessToken(
      ...
      // 加上新的参
      nowHerf = `${nowHerf}&name_en=${this.state.contentData[this.state.currentIndex]?.name_en
        }&protocal_id=${this.state.contentData[this.state.currentIndex]?.protocal_id
        }`;
