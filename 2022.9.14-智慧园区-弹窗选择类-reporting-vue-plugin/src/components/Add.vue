...
export default {
  ...
  created() {
    if (process.env.NODE_ENV !== "development") {
      ...
      // 判断表格状态
      columnStyle.choiceType == "单选" ? (this.tableChoiceType = true) : (this.tableChoiceType = false);
      // 资产ID
      this.assetId = columnStyle.assetId;
      // 拼接字段
      this.inputSplicingField = columnStyle.splicingField;
      // 拼接规则
      this.inputSplicingRules = columnStyle.splicingRules;
      // 展示字段
      this.tableShowField = JSON.parse(columnStyle.showField);
      // 编辑回显
    } else {
