<template>
  <div class="secondary_bigscreen zdfyjgtj" :style="{
    ...
    <div>
      ...
      <el-select size="small" v-model="citySelect" popper-class="inputSelectBacPoper" placeholder="请选择"
        :disabled="Boolean(city)" class="inputSelectBac" @change="changeCity" value-key="city_id" style="width: 120px">
    ...
    <el-table :data="tableData" stripe max-height="600px"
      ...
      <el-table-column prop="substation_name" label="站点名称">
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" :content="scope.row.substation_name" placement="top">
            <div class="tooltip_table">{{ scope.row.substation_name }}</div>
          </el-tooltip>
        </template>
      </el-table-column>
...
import { queryDropDownBox, stationPollingStatistics } from "../../api/asset";
import { RadioButton, RadioGroup, Tooltip, Table, TableColumn, Pagination, Input, DatePicker, Select, Option } from "element-ui";
import Vue from "vue";
import "./index.css";
Vue.use(RadioGroup);
Vue.use(RadioButton);
