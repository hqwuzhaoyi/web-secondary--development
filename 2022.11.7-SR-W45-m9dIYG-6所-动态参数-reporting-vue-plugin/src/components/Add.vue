              ...
              <div v-for="e in item.descTmplDetails" :key="e.data_id">
                <el-col :span="formColList || 24">
                  <el-form-item :prop="item.name + '.' + e.descItem.name" :label="e.descItem.name" :rules="{ required: true, message: '必填项不能为空', trigger: 'blur' }">
                    <!-- type1 输入框 -->
                    <el-input v-if="e.descItem.value_type == '1'" v-model="dataForm[item.name][e.descItem.name]" @change="formChange" :maxlength="e.descItem.value_length" :placeholder="`请输入${e.descItem.name}`" clearable style="width: 100%"></el-input>
                    <!-- type2 数字框 -->
                    <el-input-number v-if="e.descItem.value_type == '2'" v-model="dataForm[item.name][e.descItem.name]" @change="formChange" :maxlength="e.descItem.value_length" :placeholder="`请输入${e.descItem.name}`" clearable controls-position="right" style="width: 100%"></el-input-number>
                    <!-- type3 日期框 -->
                    <el-date-picker v-if="e.descItem.value_type == '3'" v-model="dataForm[item.name][e.descItem.name]" @change="formChange" align="right" type="date" placeholder="请选择日期" style="width: 100%"></el-date-picker>
                    <!-- type4 日期时间框 -->
                    <el-date-picker v-if="e.descItem.value_type == '4'" v-model="dataForm[item.name][e.descItem.name]" @change="formChange" type="datetime" placeholder="请选择日期时间" style="width: 100%"></el-date-picker>
                    <!-- type5 下拉框 -->
                    <el-select v-if="e.descItem.value_type == '5'" v-model="dataForm[item.name][e.descItem.name]" @change="formChange" placeholder="请选择" clearable style="width: 100%">
...
export default {
  ...
  methods: {
    ...
    // 触发动作
    do_EventCenter_passInParams({ value }) {
      // 获取动态参数
      this.getDynamicParams(value, "action");
    },
