  ...
  <div :id="identification" style="width: 100%; height: 100%; background-color: #fff" :ref="identification">
    <div class="settlement-details">
      <div class="button-line">
        <img src="../pluginTemp/images/export.png" style="cursor: pointer" @click="exportExcel" alt="" />
      </div>
      <div class="top_Title">光伏电费结算明细单</div>
      <div class="top_Info">
        <div style="width: 160px; color: #000" class="info-label">发电单位</div>
        <div style="width: 340px; color: #000">{{ this.excelAllData.generating_unit }}</div>
        <div style="width: 210px; color: #000" class="info-label">发电单元名称</div>
        <div style="width: 450px; color: #000">{{ this.excelAllData.power_station }}</div>
      ...
      <div class="top_Info2">
        <div style="width: 160px; color: #000" class="info-label">用电单位</div>
        <div style="width: 340px; color: #000">{{ this.excelAllData.electricity_user }}</div>
      ...
      <div class="topTable">
        <el-table :data="tableDataTop" border show-summary :summary-method="getTopSummaries" :header-cell-style="headerCellStyle" :cell-style="cellStyle">
          <el-table-column prop="dnbmc" label="电能表名称" width="160" />
          <el-table-column prop="jjlx" label="计量类型" width="150" />
          ...
          <el-table-column prop="frezz_time" :formatter="formatDate" label="抄表日期" width="140" />
      ...
      <div class="bottomTable">
        <el-table :data="tableDataBottom" border class="bottomTable" show-summary :summary-method="getBottomSummaries" :header-cell-style="headerCellStyle" :cell-style="cellStyle">
          <el-table-column prop="jl_name" label="计量点" width="160" />
          <el-table-column prop="price_avg" :render-header="renderHeader" :formatter="myfixedPrice_avg" label="平均电价|(元/kWh)" width="150">
            <!-- <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    myfixedPrice_avg(row) {
      row.price_avg = this.myFixed(Number(row.fees_all) / Number(row.poweruse_all), 5);
      if (row.price_avg == Infinity ? "0" : Number(row.fees_all).toFixed(5)) {
