...
export default {
  ...
  mounted() {
    ...
    let { 系列颜色, 图例名称颜色, 图例名称大小, 图例名称字体, 图例数值颜色, 图例数值大小, 图例数值字体, 饼图左距离, 饼图顶距离, 饼图缩放倍率, 图例右距离, 图例顶距离 } = this.options?.externalVariables
    ...
    // console.log('this.pieData', this.pieData)
    console.log('this.pieScla', this.pieScla)
  ...
  methods: {
    drawPie() {
      ...
      function getParametricEquation(startRatio, endRatio, isSelected, isHovered, k, h) {
        ...
        const offsetX = isSelected ? Math.cos(midRadian) * 0 : 0
        const offsetY = isSelected ? Math.sin(midRadian) * 0 : 0
      ...
      const option = getPie3D(self.pieData, 0)
      ...
      self.myChart.on('mouseover', function (params) {
        ...
        if (hoveredIndex === params.seriesIndex) {
          ...
          if (params.seriesName !== 'mouseoutSeries') {
            ...
            option.series[params.seriesIndex].parametricEquation = getParametricEquation(startRatio, endRatio, isSelected, isHovered, k, 30) //option.series[params.seriesIndex].pieData.value + 5
